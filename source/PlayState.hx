...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch(stageCheck)
		{
			...
			case 'round2Stage':
				{	
				defaultCamZoom = 0.9;
				curStage = 'ROUND2stage';

				var back:FlxSprite = new FlxSprite(0, 200).loadGraphic(Paths.image('Round2Stage/back'));
				back.antialiasing = true;
				back.setGraphicSize(Std.int(back.width * 1.4));
				back.scrollFactor.set(0.85, 0.85);
				back.active = false;
				add(back);

				var bg:FlxSprite = new FlxSprite(-200, 90).loadGraphic(Paths.image('Round2Stage/floor4'));
				bg.antialiasing = true;
				...
				var flor3:FlxSprite = new FlxSprite(-200, -170).loadGraphic(Paths.image('Round2Stage/floor3'));
				flor3.setGraphicSize(Std.int(flor3.width * 1.6));
				flor3.updateHitbox();
				flor3.antialiasing = true;
				flor3.scrollFactor.set(0.6, 0.6);
				flor3.active = false;

				add(flor3);

				var flor2:FlxSprite = new FlxSprite(-250, -160).loadGraphic(Paths.image('Round2Stage/floor2'));
				flor2.setGraphicSize(Std.int(flor2.width * 1.6));
				flor2.updateHitbox();
				flor2.antialiasing = true;
				flor2.scrollFactor.set(0.8, 0.8);
				flor2.active = false;

				add(flor2);
				
				var flor1:FlxSprite = new FlxSprite(-250, -140).loadGraphic(Paths.image('Round2Stage/floor1'));
				flor1.setGraphicSize(Std.int(flor1.width * 1.6));
				flor1.updateHitbox();
				flor1.antialiasing = true;
				flor1.scrollFactor.set(.99, .99);
				flor1.active = false;
		...
		kadeEngineWatermark = new FlxText(4,healthBarBG.y + 50,0,SONG.song + " - " + CoolUtil.difficultyFromInt(storyDifficulty) + " HIPPO  /  CLEAN ", 16);
		...
		if (isStoryMode)
		{
			switch (StringTools.replace(curSong," ", "-").toLowerCase())
			{
				...
				{
					if (curSong.toLowerCase() == 'groovy-fun')
				{
					add(blackFuck);
		...
		else
		{
			if (isStoryMode)
			{
				...
				if (storyPlaylist.length <= 0)
				{
					...
					else if (curSong.toLowerCase() == 'race-with-the-devil' && FlxG.save.data.soundTestUnlocked)
					{
						var video:MP4Handler = new MP4Handler();
						video.playMP4(Paths.video('ending'), new MainMenuState()); 
						//FlxG.switchState(new MainMenuState());
					}
					else if (curSong.toLowerCase() == 'race-with-the-devil' && storyDifficulty == 2 && !FlxG.save.data.soundTestUnlocked)
					{
						var video:MP4Handler = new MP4Handler();
						video.playMP4(Paths.video('ending'), new UnlockScreen(true, 'soundtest')); 
						...
						{
							var video:MP4Handler = new MP4Handler();
							video.playMP4(Paths.video('ending'), new UnlockScreen(false, 'soundtest')); 
			...
			else
			{
				...
				PlayStateChangeables.nocheese = true;
				if (curSong.toLowerCase() == 'groovy-fun' || curSong.toLowerCase() == 'execution')
				{
					PlayStateChangeables.nocheese = true;
	...
	*/
	...
	override function stepHit()
	{
		...
		if (dad.curCharacter == 'exe' && SONG.song.toLowerCase() == 'too-slow')
			{
				switch (curStep)
				{
					case 765:
		...
		if (dad.curCharacter == 'sonicfun' && SONG.song.toLowerCase() == 'groovy-fun')
			{
				switch (curStep)
				{
					case 10:
		...
		if (dad.curCharacter == 'exe' && SONG.song.toLowerCase() == 'too-slow' && curStep == 791)
			{
				shakeCam = false;
				shakeCam2 = false;
			}
		...
		if (curStage == 'ROUND2stage' && curStep != stepOfLast && FlxG.save.data.jumpscares)
			{
				switch (curStep)
				{
					case 48:
